sap.ui.define([
	...
	QUnit.test("Should always return a sorter when sorting", function (assert) {
		...
		assert.strictEqual(this.oGroupSortState.sort("PARTNER.PARTNERID").length, 1, "The sorting by PARTNER.PARTNERID returned a sorter");
	});
	...
	QUnit.test("Should set the grouping to None if the user sorts by PARTNER.PARTNERID and there was a grouping before", function (assert) {
		// Arrange
		this.oModel.setProperty("/groupBy", "GROSSAMOUNT");

		this.oGroupSortState.sort("PARTNER.PARTNERID");
